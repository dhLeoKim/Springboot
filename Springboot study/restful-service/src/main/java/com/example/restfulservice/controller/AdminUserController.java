...
import com.example.restfulservice.domain.AdminUser;
import com.example.restfulservice.domain.AdminUserV2;
...
//    @GetMapping(value = "/users/{id}", params = "version=1")
    ...
    public MappingJacksonValue retrieveUserAdmin(@PathVariable int id) {
        ...
        if (user == null) {
            ...
            BeanUtils.copyProperties(user, adminUser);  // 객체의 설정값들 복사
        }
...
//    @GetMapping(value = "/users/{id}", params = "version=2")
    ...
    public MappingJacksonValue retrieveAllUsersAdmin() {
        ...
        // 특정 필드만 포함하는 필터 생성 -> UserInfo 라는 id에 필터 적용
        SimpleBeanPropertyFilter filter = SimpleBeanPropertyFilter.filterOutAllExcept("id", "name", "joinDate", "ssn");
        FilterProvider filters = new SimpleFilterProvider().addFilter("UserInfo", filter);
